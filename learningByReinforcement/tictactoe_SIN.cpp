								...
								int player)
{
	...
	for (auto alternative : alternatives)
	{
		...
		double probWin = (numWins + k_smoothing) / (occurrences + k3_smoothing);
		double probLoss = (numLost + k_smoothing) / (occurrences + k3_smoothing);
		   ...
		   (interactive == true && iIterations < rounds))
	{
		...
		while (iLost == false && iWon == false && thereAreAlternatives)
		{
			...
			if (interactive)
			{
				do
				{
					...
					cin >> y;
				} while (x < 1 || x > 3 || y < 1 || y > 3 || board[x - 1][y - 1] != 0);
				x--;
				y--;
				board[x][y] = 1;
		...
		for (auto pair : memory)
		{
			...
			if (pair.second)
			{
				...
				Decision *currentDecision = pair.first;
				int totalOccurrences = currentDecision->numWins + currentDecision->numLost + currentDecision->numTies;

				if (iWon)
				{
					if (currentDecision->numWins < MAXOCCURRENCES)
					{
						currentDecision->numWins++;
						totalOccurrences++; // Increment total as we've added a win
					}

					if (totalOccurrences > MAXOCCURRENCES)
					{
						// Reduce numTies or numLost if they are > MINOCCURRENCES
						if (rand() % 2 == 0)
						{ // Try to reduce numTies first
							if (currentDecision->numTies > MINOCCURRENCES)
							{
								currentDecision->numTies--;
							}
							else if (currentDecision->numLost > MINOCCURRENCES)
							{ // If numTies couldn't be reduced, try numLost
								currentDecision->numLost--;
							}
						}
						else
						{ // Try to reduce numLost first
							if (currentDecision->numLost > MINOCCURRENCES)
							{
								currentDecision->numLost--;
							}
							else if (currentDecision->numTies > MINOCCURRENCES)
							{ // If numLost couldn't be reduced, try numTies
								currentDecision->numTies--;
							}
						}
					}
				...
				{
					if (currentDecision->numLost < MAXOCCURRENCES)
					{
						currentDecision->numLost++;
						totalOccurrences++; // Increment total as we've added a loss
					}

					if (totalOccurrences > MAXOCCURRENCES)
					{
						// Reduce numTies or numWins if they are > MINOCCURRENCES
						if (rand() % 2 == 0)
						{ // Try to reduce numTies first
							if (currentDecision->numTies > MINOCCURRENCES)
							{
							...
							else if (currentDecision->numWins > MINOCCURRENCES)
							{ // If numTies couldn't be reduced, try numWins
								currentDecision->numWins--;
							}
						}
						else
						{ // Try to reduce numWins first
							if (currentDecision->numWins > MINOCCURRENCES)
							{
								currentDecision->numWins--;
							}
							else if (currentDecision->numTies > MINOCCURRENCES)
							{ // If numWins couldn't be reduced, try numTies
								currentDecision->numTies--;
							}
						}
					}
				...
				{ // Game was a tie
					if (currentDecision->numTies < MAXOCCURRENCES)
					{
						currentDecision->numTies++;
						totalOccurrences++; // Increment total as we've added a tie
					}

					if (totalOccurrences > MAXOCCURRENCES)
					{
						// Reduce numWins or numLost if they are > MINOCCURRENCES
						if (rand() % 2 == 0)
						{ // Try to reduce numWins first
							if (currentDecision->numWins > MINOCCURRENCES)
							{
							...
							{ // If numWins couldn't be reduced, try numLost
								currentDecision->numLost--;
							}
						}
						else
						{ // Try to reduce numLost first
							...
							{ // If numLost couldn't be reduced, try numWins
								currentDecision->numWins--;
							}
						}
					}
		...
		{
			if ((abs(p1_Wins - impresedComputWins) + abs(p2_Wins - impressedPseudoHumanWins) + abs(numTies - impressedTies)) > 50)
			{
				...
				gp << "plot " << gp.file1d(pts_P1) << " with lines title 'P1'" << ", " << gp.file1d(pts_P2) << " with lines title 'P2'" << ", " << gp.file1d(pts_Ties) << " with lines title 'Ties'\n";
				gp.do_flush();
#endif
				if (interactive == false)
				{
